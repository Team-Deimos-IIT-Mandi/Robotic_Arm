...
try:
    ...
    de_c=trans
    S_de_c=np.matrix([[0,-de_c[2],de_c[1]],[de_c[2],0,-de_c[0]],[-de_c[1],de_c[0],0]])
...
def int_matrix(s,lam,z):
    ...
    for i in range (0,n):
        u=s[i][0]
        v=s[i][1]
        Lp=np.matrix([[-lam/z,0,u/z,u*v/lam,-(lam**2 + u**2)/lam,v],[0,-lam/z,v/z,(lam**2 + v**2)/lam,-u*v/lam,-u]])
        Le.append(Lp)
...
def image_callback(msg):
    ...
    if p0 is not None and len(p0) > 0:
        ...
        text = f"Area: {shape_area:.2f}, Angle: {orientation:.2f}" if orientation is not None else "Area: {:.2f}".format(shape_area)
